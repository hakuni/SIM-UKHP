    ...
    DetailPenelitian: function (id) {
        ...
        $.ajax({
            ...
            success: function (data) {
                mApp.unblock("#detailPenelitian");
                $("#detailPenelitian").html(data);
                if ($("#inptMilestoneID").val() == 6) {
                    $("#btnTrx").hide();
                }
                Transaction.Init(id);
                Control.Init();
...
var Transaction = {
    ...
    Alur: function (id) {
        ...
        if (
            $("#inptMilestoneID").val() == 4 || $("#inptMilestoneID").val() == 5) {
        ...
        t = $("#divHistory").mDatatable({
            ...
            columns: [{
                ...
                },
                {
                    ...
                    template: function (t) {
                        var status = "";
                        if (t.idMilestone == 1 || t.idMilestone == 6) {
                            status = "Selesai";
                        } else {
                            if (t.durasi == null) status = "Sedang dikerjakan";
                            else
                                status =
                                t.durasi < 0 ?
                                "Selesai Terlambat" :
